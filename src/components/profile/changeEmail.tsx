...
const EnterPassword = ({ setCurrentStep }: { setCurrentStep: (step: 'password' | 'email') => void }) => {
  ...
  return (
    ...
    <ConfirmButton buttonActive={buttonActive} setIsConfirmButtonPressed={handleConfirm} />
    </View>
  );
};
...
export const ChangeEmail = ({ onClose }: { onClose: () => void }) => {
  const [currentStep, setCurrentStep] = useState<"password" | "email">("password");
