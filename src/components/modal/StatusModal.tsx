...
import StatusRender from '../../hook/StatusRender';
...
export const StatusModal: React.FC<StatusModalProps> = ({ visible, onClose }) => {
  ...
  const FeaturedTab = () => {
    const featuredStatuses = statusResponseFromBE.filter(status => status.featured);
  ...
  const EmotionsTab = () => {
    const emotionStatuses = statusResponseFromBE.filter(status => status.type === 'emotion');
  ...
  const ActivitiesTab = () => {
    const activityStatuses = statusResponseFromBE.filter(status => status.type === 'activity');
  ...
  const TopicsTab = () => {
    const topicStatuses = statusResponseFromBE.filter(status => status.type === 'topic');
